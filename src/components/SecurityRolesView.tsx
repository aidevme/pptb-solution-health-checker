...
function SecurityRolesViewComponent({ securityRoles }: SecurityRolesViewProps) {
  ...
  const renderEntityPermissionsTable = (role: SecurityRoleDetail): ReactElement => (
    <div className={styles.entityPermissionsTable}>
      ...
      <div className={styles.legend}>
        <strong>Access levels:</strong> None Â· User = own records only Â· BU = Business Unit Â· P:CBU = Parent &amp; Child BUs Â· Org = Organisation-wide
      </div>
  ...
  return (
    <div className={shared.container}>
      ...
      {/* Special Permissions Matrix â€” collapsed by default, only roles with â‰¥1 permission */}
          ...
          <AccordionHeader>
            <div style={{ display: 'flex', flexDirection: 'column', gap: tokens.spacingVerticalXXS }}>
              ...
              <Text style={{ fontSize: tokens.fontSizeBase200, color: tokens.colorNeutralForeground3 }}>
                {rolesWithSpecialPerms.length} {rolesWithSpecialPerms.length === 1 ? 'role has' : 'roles have'} special permissions â€” click to expand
              </Text>
            </div>
          </AccordionHeader>
          <AccordionPanel>
