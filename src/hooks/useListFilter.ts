 ...
 *
 * `activeFilters` is initialised once from `filterSpecs` at mount. If the caller
 * violates the stability requirement and `filterSpecs` gains a new entry after
 * mount, the new dimension will have no corresponding Set; `toggleKey` falls back
 * to `new Set()` on first use — graceful degradation, not a crash.
 *
 * @example
 * ```typescript
 * const FILTER_SPECS = [
