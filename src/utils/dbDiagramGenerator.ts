...
/**
 * Generates [dbdiagram.io](https://dbdiagram.io/d) DBML from a `HealthCheckerResult`.
 *
 * @remarks
 ...
 *
 * @param result - Completed health checker result containing entity and relationship data
 * @returns DBML string ready to paste into dbdiagram.io
 */
export function generateDbDiagramCode(result: HealthCheckerResult): string {
  const lines: string[] = [];

  // Add header comment
  lines.push('// Generated by Power Platform Solution Health Checker (PPSB)');
  ...
  for (const entityHealthChecker of result.entities) {
    const tableName = entityHealthChecker.entity.LogicalName.toLowerCase();
    if (!isSystemEntity(tableName)) {
      exportedEntities.add(tableName);
      const attrs = entityHealthChecker.entity.Attributes || [];
      if (attrs.length > 0) {
  ...
  for (const entityHealthChecker of result.entities) {
    for (const rel of entityHealthChecker.entity.ManyToOneRelationships || []) {
  ...
  for (const entityHealthChecker of result.entities) {
    const entity = entityHealthChecker.entity;
